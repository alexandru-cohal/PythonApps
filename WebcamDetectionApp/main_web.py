...
if start_button:
    ...
    while True:
        ...
        weekday_current = datetime_current.strftime("%A")
        time_current = datetime_current.strftime("%H:%M:%S")

        check, frame_bgr = camera.read()

        cv2.putText(img=frame_bgr, text=weekday_current, org=(50, 50),
                    fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=2, color=(255, 0, 0),
                    thickness=2, lineType=cv2.LINE_AA)
        cv2.putText(img=frame_bgr, text=time_current, org=(50, 80),
                    fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=2, color=(0, 0, 255),
                    thickness=2, lineType=cv2.LINE_AA)
        ...
        for contour in contours:
            ...
            boundary = cv2.rectangle(frame_bgr, (x, y), (x + w, y + h), COLOR_BOUNDING_RECTANGLE,
                                     WIDTH_BOUNDING_RECTANGLE_PX)
            if boundary.any():
        ...
        streamlit_image.image(frame_bgr)
